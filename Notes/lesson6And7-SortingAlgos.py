...
There are multiple types of Sorting Algorithms:
    - Insertion Sort
        ...
        Disadvantages
            ...
            - Not as fast or efficient as other sorting algos
    ...
    - Selection Sort
        Tries to find the smallest value and swaps it with the first element of the unsorted part.
        Repeated until all is sorted. (Full explanation in code below)
        ...
        Disadvantages
            - O(n^2) for time complexity = slow for big data sets
...
Divide and Conquer Algorithm
    ...
    This gives the solution to the bigger problem that was originally proposed.
    This is a strategy that some Sorting Algorithms use to make things simpler for the computer
    and save some time while doing so.

    Some Sorting algorithms that use this are:     
        - Merge Sort
            Uses the divide and conquer algo of making them the smallest possible problems/lists by halving the 
            ...
            Disadvantages
                ...
                - Creates more arrays to store the sorted halves so it takes more space
        
        - Quick Sort
            Uses the divide and conquer but uses pivots and partitions. Pivots are elements that are compared within the array.
            ...
            A pivot is chosen in each and then the process is repeated (recursively done) until it is fully sorted.
            The pivot is usually the last or first element in the array (also the same way when separated into partitions).
            ...
            Disadvantages
                - Worst case has a time complexity of O(n^2)
        ...
        - Counting Sort
            ...
            It runs through the array and counts how many times the value appears and stores the frequency in the counting array.
            The frequency is stored in the respective element. This frequency is then used as the position for each element in the sorted
            array. 
            ...
            counting algo [0,1,2,3,4,5,6]
                          ...
                          [1,2,3,5,5,7,8] -> cumulative frequency
            
            After reaching the cumulative frequency stage, it goes back to the original array and looks at the first value.
            It then looks at the counting array and checks which position that value is in.
            It places that value in that position (eg. 2 is in the 3rd position [num in cumulative frequency]) then it decrements it by one.
            This action is for when the list goes back to that number, it can be put into that new position.

            Advantages 
                + Faster than all comparison-based algos
        ...
        - Bucket Sort
            Sorting technique that separates the array/list into buckets with uniformly distributed elements (same amount of elements in each bucket).
            ...
            Advantages
                + Buckets are sorted and processed independently meaning they do not take a lot of time 
                + Time Complexity -> O(n+m) {if linked list then O(n*n)}
                + Stable Algorithm

            Disadvantages
                - Mainly used for small data sets or evenly distributed data sets so the bigger the data sets the less efficient the sort is
        ...
        for j in range(0,n-i-1):                    #Time -> O(n)   J is the value itself that gets checked and swapped 
            ...
            if arr[j]>arr[j+1]:                     #Time -> O(1)   Checks if the value is bigger than the next one
                arr[j],arr[j+1]=arr[j+1],arr[j]     #Time -> O(1)   If that is true it swaps them and then the J loop works again
                                                    ...
                                                    #               When i is finished completely the whole array is sorted
                                                    #
        ...
        for j in range(i+1,n):                  #Time -> O(n)   This j loop checks if there is something smaller than our min value
            if arr[j]<arr[min]:                 #Time -> O(1)   if it is true, then the min becomes that new value then j loop ends 
                min = j                         #Time -> O(1)   If false then min stays the same then j loop ends 
                                                #                  - The j loop repeats until it reaches n no matter the outcome of the if statement
...
def mergeSort(arr):
    if len(arr) > 1:
        ...
        mergeSort(leftHalf)                             #calls the function again to keep splitting each half into smaller sub-lists
        ...
        while i<len(leftHalf) and j<len(rightHalf):     #only works when both are true
            ...
            if leftHalf[i]<rightHalf[j]:                #checks if the left half element i is smaller that the right half element j (both the first in their halves [0])
...
#Counting Sort ---------------------
def countSort(arr):
    maxValue = max(arr)                 #Time -> O(1)   max value is the maximum value that appears in the array
    counts =[0]*(maxValue+1)            #Time -> O(m)   creates a counting array
    output =[0]*len(arr)                #Time -> O(n)   creates a final array

    for num in arr:                     #Time -> O(n)   count occurrences of each element
        counts[num]+=1                  #Time -> O(1)   counts the frequency of each number by incrementing the element[num] by 1
    
    for i in range(1,maxValue+1):       #Time -> O(m)   update counts to counting the position of each element in the sorted array
        counts[i] += counts[i-1]        #Time -> O(1)   Calculates the cumulative frequency as seen in the explanation in [Notes]
...
def bucketSort(arr):
    ...
    for num in arr:                                     #inserts elements into their buckets
        index = int((num-minValue)/rangeOfBucket)
        if index == bucketCount:                        #checks if the index is equivalent to the count (arrays start from 0 and hold n elements)
